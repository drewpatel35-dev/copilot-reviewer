...
// Minimal GitHub PR reviewer MVP with OpenAI backoff and small web-config support
...
const SCHEMA = z.object({
  comments: z.array(z.object({
    path: z.string(),
    // line is counted over added lines in the unified diff for that file (1-based)
    line: z.number().optional(),
    start_line: z.number().optional(),
    body: z.string(),
    suggestion: z.string().optional() // replacement code (without fences)
  })).max(100),
...
function waitSecondsFrom(headers, attempt){
  ...
  const exp = Math.min(60, 2 ** (attempt - 1))
  const jitter = Math.random()
  return Math.max(retryAfter, resetReq, resetTok, exp + jitter)
}

async function openaiChat(messages, { max_tokens = 600 } = {}){
  const body = { model: MODEL, messages, temperature: TEMPERATURE, max_tokens }
...
function githubContextFromEnv(){
  // Handles both pull_request and workflow_dispatch with inputs.pr
  const repoFull = process.env.GITHUB_REPOSITORY
  if (!repoFull) throw new Error("GITHUB_REPOSITORY not set")
  const [owner, repo] = repoFull.split("/")

  let pull_number = undefined

  // Try pull_request env
  ...
  try {
    ...
    if (path) {
      ...
      if (!pull_number && ev?.pull_request?.number) {
        pull_number = Number(ev.pull_request.number)
      }
      if (!pull_number && ev?.inputs?.pr) {
        pull_number = Number(ev.inputs.pr)
      }
    }
  } catch { /* ignore */ }
...
function globToRegex(glob){
  // very small subset ** and * only
  let re = glob
    .replace(/[.+^${}()|[\]\\]/g, "\\$&")
    .replace(/\*\*/g, "§§§")       // temp token
    .replace(/\*/g, "[^/]*")
    .replace(/§§§/g, ".*")
...
async function getJSONConfig(octokit, owner, repo){
  ...
  try {
    ...
    console.warn("config.json present but not valid JSON, ignoring")
    return null
  }
}
...
async function main(){
  ...
  // Load optional config and prompt
  ...
  try {
    ...
    console.error("Model output failed schema validation. Raw content follows:")
    console.error(content)
    throw e
  }

  // Respect caps and flags
  if (!TESTS_ENABLED) parsed.tests = []
  if (!DOCS_ENABLED) parsed.docs = []
  if (parsed.comments.length > MAX_COMMENTS) {
    parsed.comments = parsed.comments.slice(0, MAX_COMMENTS)
  }
  ...
  // Prepare review comments with positions when possible
  ...
  // Post a single review for positioned comments
  ...
  // Fallback issue comment for any that we could not anchor
  ...
  // If there are tests or docs, commit them onto the PR branch
  if (parsed.tests.length || parsed.docs.length) {
    ...
    // Build a tree on top of headSha
